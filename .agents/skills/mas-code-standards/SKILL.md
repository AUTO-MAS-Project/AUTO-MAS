...
7. Before completing a user-visible feature or fix, add its `res/version.json` entry under the next unreleased version; treat the entry as part of the implementation rather than a reminder. Add exactly one entry per PR — condense all of the PR's changes into a single entry.
