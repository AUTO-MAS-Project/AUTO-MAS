...
所有专项统一提供配置来源选择（缺省三态：脚本 / 用户 / 直控，**例外见下表**：MaaFW 仅用户、Whimbox 两态）。三态只决定配置 owner；各专项的物理落盘、会话和运行方式仍按真实架构确认，不机械复制目录或配置模型。**MaaFW 不是专项**（通用引擎，任何 `interface.json` 项目都由它运行），三态对它没有所指，见 `app/task/MaaFW/AGENTS.md`。
...
| BAAH | 脚本 / 用户 / 直控 三态 |
| Whimbox | 脚本 / 直控 **两态**（上游不按账号分档配置、MAS 也无法指定运行账号，无「用户」态）；**不支持快速配置**（与 ZzzOd 同因，见下） |
...
Whimbox 没有「用户」态与快速配置，两者**理由不同**：无「用户」态是因为**上游不按账号配置、MAS 也无法指定运行账号**（账号由上游自行识别与轮换），用户维度既不能配也不能指定跑；无快速配置则是因为快速配置的唯一消费点（直控态注入面板字段）与 Whimbox「直控 = MAS 零写入」的定位相悖（与 ZzzOd 同因），且「脚本」态面板覆盖的本来就是上游一条龙参数全集，没有可划的高频子集。
