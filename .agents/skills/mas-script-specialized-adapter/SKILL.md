...
- **侧车预览双分区**：「MAS 独有配置」在前（MAA GUI 无对应概念、查看详细配置看不到，**全量**：配置文件来源/关卡配置模式/剿灭开始星期/活动关优先+选关意图+理智药/绿票商店/库存保持计划），「MAA 配置」在后（与 MAA GUI 同口径）。
- **活动关意图词表口径**：侧车预览、状态行与提示文案用全称「倒数第N关」，槽位列与下拉选项前缀用简称「倒N」「搓玉」——同一 `jade`/`last:N` 值在两种场景各取一种写法，不是文案不一致，统一时别把窄列撑开。
- **活动关序号→意图的旧值翻译**：旧序号是「MAA 列表位置」，`last:N` 是「按编号降序的倒数名次」，两者同形不同义，因此如实迁成 `pos:N` 而不是近似成 `last:N`——同一期选过的「倒3」在下期只有 2 个材料关时就与旧序号同形，近似迁移会让它被旧值兜底认领、静默改刷其它关。配置的 `legacy_name` 迁移、侧车 `group_overlay` 与预览翻译、`ActivityStageIntentValidator` 都按这个口径做，`_resolve_activity_stage` 按列表位置解析 `pos:N`，越界不回退首关。总开关没开时旧序号只是默认值 1（v5.4.0 起默认即落盘），不迁（保持未指派），免得没碰过活动关的用户全被算成已指派。
- **活动关跳过簿是「连续出错」**：首错跳当日、再错（中间没打成功）跳至活动结束，打成功即清条目（#868）；统计与文案都说「连错」，别把它当成跨日累计。
