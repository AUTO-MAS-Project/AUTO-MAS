...
- **活动关序号→意图的旧值翻译**：旧序号是「MAA 列表位置」，`jade`/`last:N` 是「按编号降序的倒数名次」，两者只在列表按编号降序时逐位等价。配置的 `legacy_name` 迁移、侧车 `group_overlay` 与预览翻译、`ActivityStageIntentValidator` 都按这个近似规则做；指向末位玉关的旧值必然越界，由 `_resolve_activity_stage` 的旧序号兼容分支按搓玉解析——该分支只为旧配置/旧备份保留（新 UI 的倒数上限是 `len(ranked)`，产生不出该值），不要当越界回退清理掉。
