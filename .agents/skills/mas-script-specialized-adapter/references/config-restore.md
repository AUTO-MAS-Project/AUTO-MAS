...
- **mas 池**：优先展示**与页面表单同源的内容**。页面字段不落盘在被备份文件
  ...
  对象），展示文件内页面管理的关键字段。侧车字段建议按**「原生对应 / 专项
  独有」分区**展示：独有区是「查看详细配置」（打开原生 GUI）看不到的信息，
  必须**全量**进预览；原生对应区与原生池同口径。关卡类多槽位字段（主关卡+
  备选）合成**单一「具体刷什么本」行**，与配置界面的折叠摘要同口径（全部
  槽位禁用 = 原生默认行为，如 MAA 的当前/上次）；哨兵值翻译以下拉/界面的
  实际标签为准，不臆造。影响恢复目标或危险回填的字段（如 MAA 配置文件
  来源）只进预览、不参与回填。
- **native 池**：展示脚本原生配置里**结构稳定、用户关心的字段**；随版本漂移、
  ...
  **与 mas 池同字段必须同口径**——同一配置概念（任务开关、服务器、账号、
  ...
  TaskQueue 反读出与侧车一致的「自动唤醒/理智作战/吃理智药」行）；仅隶属
  一侧的字段（MAA 的当前方案/连接地址/启动游戏）才单独展示，两池行集因此
  允许不同，但重叠部分逐字一致。
