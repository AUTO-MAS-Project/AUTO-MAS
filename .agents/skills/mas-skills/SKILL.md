...
12. `mas-community`: community-tool semantics across credentials, role discovery, daily activity, sign scheduling, notifications, provider capability evidence, and compatibility migration.
...
Use `mas-game-sign`.
13. Task mentions community plugins/tools, role discovery, daily activity or memo, provider capability references, community notification aggregation, or migration from `game_sign` semantics:
Use `mas-community`, and combine `mas-game-sign` when the change also touches the actual sign-in flow.
