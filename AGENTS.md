...
- 用户可见的功能或问题修复必须在实现完成前更新 `res/version.json`，不得只提醒用户；按文档站规范将条目写入下一个未发布版本，**一条 PR 只贡献一条 changelog 记录，并必须用一句最简洁的语言概括该 PR 的意义**，将全部改动合并为一句话写入匹配的分类下。除非用户明确要求，不要为此自行创建提交。
