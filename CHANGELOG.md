...
- (绝区零一条龙) 升级后，旧版本存在脚本目录的原生配置备份不再显示在配置恢复列表，数据未删除 by @AthenaHibou

### 新增

- (MAA) 用户配置新增干员养成目标，按材料缺口自动刷取，达成后自动移除并通知 (#721) by @jinghero
- (MAA) 养成计划可绑定森空岛，按当天可刷取的材料缺口刷取；库存保持展示本用户识别档案 (#790) by @jinghero
- (BetterGI) 一条龙配置区改为「配置组」并新增「录制」标签页，录制可作为独立队列项 (#653) by @TCddddd
- (BetterGI) 新增「队伍配置」：秘境、地脉花、首领讨伐可分别指定队伍与战斗策略 (#731) by @TCddddd
- (绝区零一条龙) 用户设置新增启动参数配置（含 DX12 独立开关），与一条龙原生设置同源读写 (#814) by @AthenaHibou
- (BAAH) 首页新增碧蓝档案活动卡片，可切换日服 / 国际服 / 国服查看进行中的活动 (#707) by @beichen24a1
...
- (Runtime) 初始化失败时始终保留查看日志入口，日志页支持打包日志 (#742) by @Craun718

### 变更

- (MaaEnd) 改用内置切号与游戏启动设置，结束关闭游戏时可恢复预设或自定义分辨率 (#748) by @HarcoChen
- (MaaEnd) 用户页重新整理：分离基本信息与配置来源，简化任务分类并收起低频设置 (#764) by @HarcoChen
- (MFW) 建脚本时「MFW」挪到通用脚本一栏，说明改为运行任何带 interface.json 的项目 (#786) by @qiyinxi
- (MFW) 用户页去掉「配置来源」三态与「快速配置」开关，任务队列始终显示 (#788) by @qiyinxi
- (模拟器) 「大雷主人模式」补齐宿主层：雷电加载页轮播与开机全屏页不再出现，关闭时还原 (#804) by @qiyinxi
...
- (MaaEnd) 修复脚本设置在 MaaEnd 配置目录缺失时额外报「配置换入的源目录不存在」的问题 (#752) by @Taimer0721
- (MaaEnd) 修复指定理智关卡可能不生效的问题，并按安装版本动态显示自动采集路线 (#745) by @HarcoChen
- (MaaEnd) 修复游戏退出后再次启动时误报失败并浪费一次重试的问题 by @HarcoChen
- (MaaEnd) 修复重试时带入失效任务、无可执行任务时等待超时的问题 by @HarcoChen
...
- (BetterGI) 修复自动秘境未刷够轮次就结束、「刷取至树脂耗尽」被当作限定次数的问题 [仅公测] (#753) by @TCddddd
- (BetterGI) 合并 9 个改动：通知走 MAS 编排、进程杀不掉与提权重启误判、地脉花策略等一批修复 [仅公测] (#787) by @TCddddd
...
- (设置) 虚拟显示器：任务期间真实显示器接回时弹窗询问是否拆除，未选择则任务结束后拆 (#812, #802) by @qiyinxi
- (Runtime) 初始化遇到仓库异常时直接说明需要删除的文件和重新下载方法 by @ClozyA
...
- (MFW) 同一个任务可以重复加入任务队列，每一份各自配置选项与排序 (#660) by @qiyinxi
- (模拟器) Emulator 2.0 支持「大雷主人模式」，并新增「打开游戏中心」按钮 (#675) by @qiyinxi
- (Runtime) 初始化下载前先对各下载源测速并按最快顺序使用，安装依赖时显示进度与速度 (#678) by @qiyinxi
...
- (MaaEnd) 修复脚本更新移除旧任务后，自动代理重试反复报「没有启用的任务」并卡住的问题 (#672) by @1w1w11w1
...
- (绝区零一条龙) 用户卡片新增来源、区服、账号尾号与一条龙任务数标签 by @AthenaHibou
- (MFW) 使用模拟器时可顺带打开游戏（包名自动识别或手动填写）；任务前后自定义脚本每用户各跑一次 (#638) by @qiyinxi

### 变更

- (MFW) MaaFramework 原生日志备份改为按保留天数一并清理，不再无限堆积 (#640) by @qiyinxi
- (设置) 虚拟显示器改为常驻监测：检测不到真实显示输出自动挂上，恢复后自动拆掉 by @qiyinxi
...
- (Runtime) 修复配了网络代理的用户初始化下载仍直连、卡在「程序文件」一步的问题 by @qiyinxi
- (Runtime) 修复后端就绪前退出时只能反复重试的问题，同时提供「重建运行环境」入口 by @qiyinxi
...
- (MFW) 新增自动更新时机设置，已有脚本默认「运行前更新」，可选下载源与更新通道 by @qiyinxi by @TCddddd
...
- (MaaEnd) 新增脚本直控模式与每日仅执行一次，抢委托送货与自动采集独立为可单独配置的阶段 by @HarcoChen by @TCddddd
- (HSR) 用户配置页新增「额外脚本」，可在任务前后各执行一个自定义脚本 by @qiyinxi by @TCddddd
- (BetterGI) 用户编辑页一条龙配置改为可视化拖拽队列，战斗四项各支持多个独立实例 by @TCddddd
...
- (MFW) 进入项目配置页不再每次等运行环境确认；通用脚本未填配置路径时任务前直接提示 by @qiyinxi
...
- (MaaEnd) 修复更新后已完成任务被误判失败并重试的问题 by @HarcoChen by @TCddddd
- (HSR) 修复任务配置文本项被清空、开启培养目标后历战余响不刷、脚本升级后表单消失等问题 by @qiyinxi by @TCddddd
- (BetterGI) 修复一条龙配置级联退化与保存报错、跨零点日志误判、切号误删仓库等问题 [仅公测] by @TCddddd by @1w1w11w1 by @Craun718 by @qiyinxi
...
- (Runtime) 修复启动界面日志窗口空白、深色模式标题栏变暗、双窗口抢连接、渲染崩溃黑屏 by @qiyinxi by @ClozyA by @TCddddd

### 开发流程

- 统一导入排序、合并重复逻辑、清理死代码；前端类型治理；BetterGI 编辑页接入 i18n by @1w1w11w1 by @HarcoChen by @qiyinxi
...
- (调度) 新增每日首次启动运行队列；托盘右键菜单可自定义；新增 BetterGI 原神专项适配 by @luo-luo-o by @1w1w11w1 by @TCddddd by @qiyinxi
...
- (MFW) interface 解析接上缓存（用户页进入 5.5s→0.12s）；单独运行不再受单日次数上限约束 by @qiyinxi
...
- (MaaEnd) 修复脚本退出后任务持续等待问题 by @HarcoChen by @qiyinxi
