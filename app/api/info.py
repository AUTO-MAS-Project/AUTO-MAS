...
async def get_stella_activity() -> InfoOut:
    ...
    格式转换仍由前端完成，与碧蓝档案那条链路一致。取数失败返回一份空数据，由
    卡片显示自己的失败态，不影响其它卡片。
    ...
    Returns:
        InfoOut: 站点原始响应，另加 ``official`` 横幅列表；取不到排期时 data 为空字典。
    """
    ...
    matched = match_official_banner(banners or [], current_activity_name(payload))
    if matched is not None:
        matched["matched"] = True
    return InfoOut(data={**payload, "official": banners or []})
