...
class SimpleUserConfig(ConfigBase):
    """简易脚本用户配置"""
...
class SimpleConfig(ConfigBase):
    ...
    def __init__(self) -> None:
        ...
        ## Info ------------------------------------------------------------
        ## 脚本名称
        self.Info_Name = ConfigItem("Info", "Name", "新简易脚本")
        ## 根目录路径
        self.Info_RootPath = ConfigItem(
            "Info", "RootPath", "", FileValidator()
        )
        ...
        ## 脚本参数
        self.Script_Arguments = ConfigItem(
            "Script", "Arguments", "", ArgumentValidator()
        )
        ## 是否追踪进程
        self.Script_IfTrackProcess = ConfigItem(
            "Script", "IfTrackProcess", False, BoolValidator()
        ...
        ## 单次运行总时长限制（分钟），0 表示不限制
        self.Run_RunTimeLimit = ConfigItem(
            "Run", "RunTimeLimit", 10, RangeValidator(0, 9999)
        )

        self.UserData = MultipleConfig([SimpleUserConfig])
...
class GlobalConfig(ConfigBase):
    ...
    def __init__(self):
        ...
        SimpleConfig.related_config["EmulatorConfig"] = self.EmulatorConfig
