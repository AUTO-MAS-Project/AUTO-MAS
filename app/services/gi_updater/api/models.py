...
class HypPackageData:
    ...
    language: str = ""
    version: str = ""
    pkg_version_file_name: str = ""
    channel_sdk_pkg: str = ""
    run_command: str = ""
    validation: List["HypPackageData"] = field(default_factory=list)
    ...
    def from_json(cls, data: Dict[str, Any]) -> "HypPackageData":
        ...
        Returns:
            解析后的 ``HypPackageData``；``run_command`` 取的是 JSON 的 ``command`` 字段。
        """
        return cls(
            ...
            decompressed_size=_int(data.get("decompressed_size")),
            language=_str(data.get("language")),
            version=_str(data.get("version")),
            pkg_version_file_name=_str(data.get("pkg_version_file_name")),
            channel_sdk_pkg=_str(data.get("channel_sdk_pkg")),
            run_command=_str(data.get("command")),
            validation=[cls.from_json(item) for item in (data.get("validation") or [])],
        )
    ...
    def file_name(self) -> str:
        """从有效 URL 末段取文件名（``effective_url`` 的最后一段）。"""
        return self.effective_url.rsplit("/", 1)[-1]
