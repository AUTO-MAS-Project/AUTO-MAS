...
class GenshinInstaller(InstallManagerBase):
    ...
    def migrate_audio_directory(self) -> int:
        ...
        Returns:
            实际搬移的文件数；非原神版本或旧目录不存在时返回 0。
        """
        version = self.version
        if not isinstance(version, GameTypeGenshinVersion):
            return 0
    ...
    def validate_exec_data_dir(self) -> bool:
        ...
        Returns:
            非原神版本恒为 True；原神版本委托
            ``version.is_exec_data_dir_valid()`` 判定（混装时为 False）。
        """
        if isinstance(self.version, GameTypeGenshinVersion):
    ...
    def candidate_executable_names(self) -> List[str]:
        ...
        Returns:
            原神版本委托 ``version._candidate_executable_names()``；
            非原神版本回退到 ``preset.executable_name`` 的单元素列表。
        """
        if isinstance(self.version, GameTypeGenshinVersion):
