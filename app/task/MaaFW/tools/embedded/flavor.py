...
``decorate_selection``、在导入项目后用 ``matches_project`` 决定脚本类型、在写用户任务快照前
调一次可选的快照整理钩子。这里不出现任何具体专项的名字；专项自己的逻辑全在它自己的包里。

可选钩子 ``sanitize_task_snapshot``（写用户任务快照前整理）的契约：

- **可选**：不写进 ``MaaFWFlavor`` 协议——``runtime_checkable`` 的协议会把缺它的特调判成
  不满足协议、整个退回通用 MaaFW。``resolve_snapshot_sanitizer`` 按 ``getattr`` 探测。
