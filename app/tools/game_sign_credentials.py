...
from .game_sign_contract import CredentialStatus
...
def parse_game_sign_credential(
    ...
    text = text.strip()
    if not text:
        return GameSignCredentialInfo(token_field, "empty")

    if token_field == "MiyousheToken":
        fields = _cookie_field_names(text)
        if fields:
            return GameSignCredentialInfo(
                token_field, "cookie", fields, configured=True
            )

    fields = _json_field_names(text)
    if fields is not None:
        return GameSignCredentialInfo(token_field, "json", fields, configured=True)

    # 保留旧版纯 Token、损坏 JSON 和未知平台 Token 的“已配置”语义。
    return GameSignCredentialInfo(token_field, "opaque", configured=True)


def is_game_sign_credential_configured(token_field: str, raw_token: object) -> bool:
    """判断凭据是否非空，不对平台有效性作结论。"""

    return parse_game_sign_credential(token_field, raw_token).configured
...
def validate_game_sign_credential(
    token_field: str, raw_token: object
) -> CredentialStatus:
    """执行提供方本地凭据校验，不代表上游 Token 仍然有效。"""

    info = parse_game_sign_credential(token_field, raw_token)
...
# 新代码使用社区凭据语义；旧名称继续作为历史调用兼容入口。
CommunityCredentialInfo = GameSignCredentialInfo
...
validate_community_credential = validate_game_sign_credential
