...
SMS_SEND_URL = "https://api.kurobbs.com/user/getSmsCode"
SMS_LOGIN_URL = "https://api.kurobbs.com/user/sdkLogin"
KURO_SMS_SESSION_TTL = 600

# 短信验证码只用于当前登录请求；进程重启或会话过期后不会继续有效。
_KURO_SMS_SESSIONS: dict[str, _KuroSmsSession] = {}
...
def validate_kuro_sms_phone(phone: str) -> str:
    """校验并规范库街区短信登录手机号。"""

    value = str(phone or "").strip()
    if len(value) != 11 or not value.isdigit() or not value.startswith("1"):
        raise ValueError("库街区手机号格式无效")
    return value


def _validate_kuro_sms_code(code: str) -> str:
    """校验短信验证码，不在日志或异常中回显验证码内容。"""

    value = str(code or "").strip()
    if not 4 <= len(value) <= 8 or not value.isdigit():
        raise KuroSmsCodeError("库街区验证码格式无效")
    return value


def _new_kuro_sms_session(account_id: str, phone: str) -> tuple[str, _KuroSmsSession]:
    """创建不含验证码的短期服务端会话。"""

    session_id = secrets.token_urlsafe(24)
    session = _KuroSmsSession(
        account_id=account_id,
        phone=phone,
        dev_code=secrets.token_hex(20).upper(),
        distinct_id=str(uuid.uuid4()),
        expires_at=time.monotonic() + KURO_SMS_SESSION_TTL,
    )
    return session_id, session


def _prune_kuro_sms_sessions() -> None:
    """删除过期短信会话，避免长期保留手机号和设备标识。"""

    now = time.monotonic()
    for session_id, session in tuple(_KURO_SMS_SESSIONS.items()):
        if session.expires_at <= now:
            _KURO_SMS_SESSIONS.pop(session_id, None)


def _expire_kuro_sms_session(session_id: str, expires_at: float) -> None:
    """主动删除指定的到期会话，不影响同名的新会话。"""

    session = _KURO_SMS_SESSIONS.get(session_id)
    if session is not None and session.expires_at == expires_at:
        _KURO_SMS_SESSIONS.pop(session_id, None)


def _schedule_kuro_sms_session_expiry(
    session_id: str,
    session: _KuroSmsSession,
) -> None:
    """注册会话到期回调，确保手机号不会依赖后续请求才被清理。"""

    delay = max(0.0, session.expires_at - time.monotonic())
    asyncio.get_running_loop().call_later(
        delay,
        _expire_kuro_sms_session,
        session_id,
        session.expires_at,
    )


def _get_kuro_sms_session(
    session_id: str,
    account_id: str,
    phone: str,
) -> _KuroSmsSession:
    ...
    session = _KURO_SMS_SESSIONS.get(session_id.strip())
    if session is None or session.account_id != account_id or session.phone != phone:
...
def _raise_kuro_sms_response_error(
    response: httpx.Response,
    data: dict[str, object],
    stage: str,
) -> None:
    """将短信接口的 HTTP/业务错误转换为稳定的领域异常。"""

    code = data.get("code")
    if response.status_code == 429 or _is_kuro_code(code, 242):
        raise KuroSmsRateLimitError("库街区短信验证码发送过于频繁")
    if not response.is_success:
        raise KuroSmsError(
            f"库街区{stage}失败（HTTP {response.status_code}）"
        )
    if _is_kuro_code(code, -130):
        raise KuroSmsCodeError("库街区短信验证码错误或已过期")
    if not _is_kuro_code(code, 200):
        raise KuroSmsError(
            f"库街区{stage}失败（code={code}）：{_kuro_response_message(data, '上游返回失败')}"
        )


async def create_kuro_sms_session(
    account_id: str,
    phone: str,
    proxy: str | None = None,
    ...
    session_id, session = _new_kuro_sms_session(account_id, normalized_phone)
    ...
    async with httpx.AsyncClient(proxy=resolved_proxy, trust_env=False) as client:
        response = await client.post(
            ...
            data={"mobile": normalized_phone, "geeTestData": ""},
            timeout=30.0,
        )
    response_data = _safe_json(response)
    _raise_kuro_sms_response_error(response, response_data, "发送短信验证码")
...
async def login_kuro_with_sms(
    ...
    """使用短信验证码换取库街区 APP Token，并消费成功会话。"""

    normalized_phone = validate_kuro_sms_phone(phone)
    normalized_code = _validate_kuro_sms_code(code)
    normalized_session_id = str(session_id or "").strip()
    if not normalized_session_id:
        raise KuroSmsSessionError("库街区短信登录会话已失效，请重新发送验证码")
    session = _get_kuro_sms_session(
    ...
    response_data = _safe_json(response)
    _raise_kuro_sms_response_error(response, response_data, "短信登录")

    payload = response_data.get("data")
    if not isinstance(payload, dict):
        raise KuroSmsError("库街区短信登录响应数据不完整")
    token = payload.get("token")
    if not isinstance(token, str) or not token.strip():
        raise KuroSmsError("库街区短信登录未返回完整 Token")

    # 登录成功后立即消费会话；refreshToken 未被当前签到链路确认，暂不持久化。
    _KURO_SMS_SESSIONS.pop(normalized_session_id, None)
    return token.strip()
