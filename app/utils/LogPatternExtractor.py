...
def serialize_patterns(patterns: list[dict]) -> str:
    ...
    for item in patterns:
        ...
        if ptype == PATTERN_TYPE_SPLIT:
            ...
            extract = (item.get("extract") or "").strip()
            max_lines = item.get("maxLines", _MULTILINE_DEFAULT_MAX_LINES)
            try:
                max_lines = int(max_lines)
                if max_lines < 2:
                    max_lines = 2
