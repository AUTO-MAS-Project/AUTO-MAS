...
import { SmartDownloader, DownloadProgress } from './downloadService'
...
export class AppUpdateService {
    ...
    async installUpdate(): Promise<OutBase> {
        ...
        try {
            ...
            spawn(
                installerPath,
                ['/SP-', '/SILENT', '/NOCANCEL', '/FORCECLOSEAPPLICATIONS', '/LANG=Chinese', `/DIR=${this.appRoot}`],
                {
                    detached: true,
                    stdio: 'ignore',
                    windowsHide: true,
    ...
    private readUpdateSettings(): UpdateSettings {
        ...
        try {
            ...
            if (frontendConfig) {
                const data = frontendConfig
                const updateCfg = (data?.Update || {}) as Record<string, any>
                if (typeof updateCfg.Source === 'string' && ['GitHub', 'MirrorChyan', 'AutoSite'].includes(updateCfg.Source)) {
                    source = updateCfg.Source as UpdateSource
                }
                if (typeof updateCfg.Channel === 'string' && updateCfg.Channel.trim()) {
                    channel = updateCfg.Channel.trim()
                }
                if (typeof updateCfg.MirrorChyanCDK === 'string') {
                    mirrorChyanCDK = updateCfg.MirrorChyanCDK.trim()
                }
            }
        } catch (error) {
            logger.warn(`读取后端配置失败，使用默认更新设置: ${error}`)
        }

        try {
            const backendConfig = loadBackendConfigFile(this.appRoot)
            if (backendConfig) {
                const data = backendConfig
                const updateCfg = (data?.Update || {}) as Record<string, any>
                if (
                    typeof updateCfg.Source === 'string' &&
                    ['GitHub', 'MirrorChyan', 'AutoSite'].includes(updateCfg.Source)
                ) {
                    source = updateCfg.Source as UpdateSource
                }
                if (typeof updateCfg.Channel === 'string' && updateCfg.Channel.trim()) {
                ...
                if (typeof updateCfg.MirrorChyanCDK === 'string' && updateCfg.MirrorChyanCDK.trim()) {
                    mirrorChyanCDK = updateCfg.MirrorChyanCDK.trim()
                }
            }
        } catch (error) {
            logger.warn(`读取前端配置失败，忽略前端更新设置覆盖: ${error}`)
        }
    ...
    private compareVersion(a: string, b: string): number {
        const toTokens = (v: string): Array<number | string> =>
            v
                .replace(/^v/i, '')
                .split(/[.\-]/)
                .filter(Boolean)
                .map(token => {
                    ...
                    return Number.isFinite(n) && token === String(n) ? n : token.toLowerCase()
                })
