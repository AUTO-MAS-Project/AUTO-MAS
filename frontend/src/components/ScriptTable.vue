<template>
  <div class="scripts-grid">
    ...
    <draggable
v-model="localScripts" item-key="id" :animation="200" ghost-class="script-ghost"
            ...
            <div class="script-header">
              <div class="script-info">
                ...
                <div class="script-logo-container">
                  ...
                  />
                  <img
v-else-if="script.type === 'MaaEnd'" src="@/assets/MaaEnd.png" alt="MaaEnd"
                    class="script-logo"
                  />
                  <img
...
:color="script.type === 'MAA'
              ...
              <div class="header-actions">
                <a-button
v-if="script.type === 'MAA' && !props.activeConnections.has(script.id)" type="primary" ghost
                  size="middle"
                ...
                <a-button
v-if="script.type === 'MAA' && props.activeConnections.has(script.id)" type="default"
                  size="middle"
                  disabled
                ...
                <a-button
v-if="script.type === 'SRC' && !props.activeConnections.has(script.id)" type="primary" ghost
                  size="middle"
                ...
                <a-button
v-if="script.type === 'SRC' && props.activeConnections.has(script.id)" type="default"
                  size="middle"
                  disabled
                  style="color: #52c41a; border-color: #52c41a"
                >
                ...
                <a-popconfirm
title="确定要删除这个脚本吗？" description="删除后将无法恢复，请谨慎操作" ok-text="确定" cancel-text="取消"
            ...
            <div v-if="script.users && script.users.length > 0" class="users-section">
              ...
              <draggable
v-model="script.users" item-key="id" :animation="200" ghost-class="user-ghost"
                    ...
                    <div class="user-info">
                      <div class="user-details-row">
                        ...
                        >
                          <!-- 直接使用后端提供的Tag字段 -->
                          <a-tag
v-for="(tag, index) in parseStatusTagList(user.Info.Tag)" :key="index"
                            :title="tag.text"
                            :class="[
                              'info-tag',
...
v-for="(tag, index) in parseStatusTagList(user.Info.Tag)" :key="index"
                        ...
                        <div v-if="script.type === 'M9A'" class="user-info-tags">
                          <!-- 显示备注（仅当有值时）-->
                          <a-tag
v-if="user.Info.Notes && user.Info.Notes !== '无' && user.Info.Notes.trim() !== ''"
                            "
                            color="geekblue"
                            class="info-tag"
                          ...
                          <a-tag
v-for="(tag, index) in getM9AOnceStatusTags(script, user)" :key="`m9a-once-${index}`"
                            :title="tag.text"
                            class="info-tag"
                            :color="tag.color"
                          >
...
v-for="(tag, index) in parseStatusTagList(user.Info.Tag)" :key="index"
                    ...
                    <div class="user-controls">
                      <div class="user-status">
                        <a-switch
:checked="user.Info.Status" :checked-children="'启用'" :un-checked-children="'禁用'"
                          class="status-switch"
                      ...
                      <div class="user-actions">
                        ...
                        <a-popconfirm
title="确定要删除这个用户吗？" description="删除后将无法恢复" ok-text="确定" cancel-text="取消"
