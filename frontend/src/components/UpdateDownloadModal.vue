...
let hasAppUpdateListener = false
...
const startDownload = async () => {
  logger.info('开始下载流程（Electron IPC）')
  ...
  try {
    // 确保有版本信息，如果没有则先检查更新
    let targetVersion = props.latestVersion
...
const ensureAppUpdateListener = () => {
  if (hasAppUpdateListener) {
    return
  }
...
watch(
  ...
  newVisible => {
    ...
    if (newVisible) {
      logger.info('窗口显示，确保IPC事件监听并开始下载')
      ensureAppUpdateListener()
      // 开始下载
      startDownload()
    } else {
      logger.info('窗口隐藏，重置状态')
      resetState()
    }
  }
)
