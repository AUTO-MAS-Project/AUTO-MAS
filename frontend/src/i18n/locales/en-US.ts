...
export default {
  ...
  edit: {
    ...
    mssLaunchPathHint: 'The game exe to start in DirectExe mode',
    ...
    mssEmulatorHint:
      'Pick the emulator this app should schedule; it starts the emulator and waits for the device before the run, so the MSS shell only connects',
    ...
    mssCloseOnFinish: 'Close the emulator when finished',
    mssCloseOnFinishHint:
      'When on, the emulator picked above is closed after the run; when off it stays running',
    ...
    mssAvailableTasks: 'Available tasks',
    mssAvailableTasksHint:
      'The list is synced by this app from the MSS interface.json before each run; tick a task to add it to the queue below',
    mssAvailableTasksEmpty:
      'No available task list yet: check the MSS root folder in the script settings and run a task at least once',
    mssTaskQueueHint:
      'Tasks run in order and can be moved up or down; leaving an option empty keeps the value from MSS',
    mssQueueEmpty: 'No task selected yet — tick one above',
    mssOptionValueHint:
      "Select options take the option name; input options take a JSON object such as {'{\"account\": \"123\"}'}",
