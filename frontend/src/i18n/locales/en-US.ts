...
export default {
  ...
  edit: {
    ...
    // MSS dedicated adapter
    mssScriptConfiguration: 'MSS script settings',
    mssScriptNameHint: 'Distinguishes this MSS script instance from others',
    mssRootPath: 'MSS root folder',
    mssRootPathHint:
      'Pick the root folder of the MSS release (it should contain both MFAAvalonia.exe and interface.json); the instance config and log directories are derived from it',
    mssGameSection: 'Game (desktop)',
    mssLaunchModeHint:
      'DirectExe lets this app start the game and close it when the run ends; AttachOnly only takes over an already running window and neither starts nor closes it',
    mssLaunchModeDirect: 'Start it for me',
    mssLaunchModeAttach: 'Attach to a running game',
    mssLaunchPathHint:
      'The game exe to start in DirectExe mode; point at the game itself, not the MFAAvalonia.exe shell',
    mssGameArguments: 'Game launch arguments',
    mssGameWaitTime: 'Wait for the window (seconds)',
    mssUnityResolution: 'Window resolution',
    mssUnityResolutionHint:
      'Before launching, look up the Unity registry entry for this exe and temporarily switch to the chosen window size, restoring it when the game closes. Unity engine only. MSS officially supports 16:9 clients only',
    mssEmulatorUnsupported:
      'The emulator build of Stella Sora has a bug that prevents the game from launching, so this script does not support emulators; please use the desktop build. It will not be adapted unless that bug is fixed upstream',
    mssRunTimesLimitHint: 'Stop the run when this many attempts still fail',
    mssRunTimeLimitHint:
      'Longest the run may go without new log output, in minutes; exceeding it counts as a failed run',
    mssUseAdmin: 'Launch with admin rights',
    mssUseAdminHint:
      "MSS's desktop controller needs administrator rights to operate the game window; when enabled this app starts the shell elevated (no extra UAC prompt if this app is already elevated)",
    mssNotMssScript: 'This script is not an MSS script',
    mssLastProxyDate: 'Last run date',
    mssProxyTimes: 'Run count',
    mssDataReadOnlyHint: 'Tracked by this app automatically, read-only',
    mssUserTag: 'User tags',
    mssUserTagHint: 'Generated by this app from run results, read-only',
    mssOrchestration: 'Task orchestration',
    mssOrchestrationHint:
      'Before each run this app rewrites the task checkboxes and stage in the Stella Sora shell according to the settings below, then restores them afterwards',
    mssPlanMode: 'Bounty trial stage',
    mssPlanModeHint:
      '"Default" keeps the stage configured in the shell; picking a plan follows the stage for the current day',
    mssPlanFixed: 'Default (use the shell setting)',
    mssActivityFirst: 'Event first',
    mssActivityFirstHint:
      'When on, the event battle runs first during an event and the bounty trial then follows the setting above; when off the event task is left alone',
    mssClimbMode: 'Weekly tower climb',
    mssClimbModeHint:
      'When enabled, the new tower climb is farmed once a week; it will not repeat in the same week',
    mssClimbClose: 'Off',
    mssClimbAuto: 'Auto',
    mssClimbStartWeekday: 'Weekly start day',
    mssClimbStartWeekdayHint:
      'The weekly task will not run before this day; e.g. Wednesday means Monday and Tuesday are skipped',
    mssClimbTimes: 'Climb count',
    mssClimbTimesHint:
      'How many climbs to run each week, written to the "climb count" input of the new tower climb',
    mssClimbCompletedWeek: 'Weekly completed week',
    weekday: {
      Monday: 'Monday',
      Tuesday: 'Tuesday',
      Wednesday: 'Wednesday',
