...
export default {
  ...
  edit: {
    ...
    mssFlavorQueueHint:
      'タスクキューが空でプランも選んでいない場合、実行できるタスクがありません。少なくとも「懸賞試練クイックバトル」にチェックを入れるか、プランを選んでください（プランを選ぶと自動で追加されます）。イベントのクイックバトルはイベント期間中のみ、懸賞試練より先に実行します。新しい塔登りは最後に回します。週に一度だけ実行するには、スクリプトの「実行設定」の「今週完了したらスキップ」に追加してください',
    mssFlavorPlanHint:
