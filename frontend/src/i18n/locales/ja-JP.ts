...
export default {
  ...
  edit: {
    ...
    mfwStartupSettleSeconds: '起動後の最大待機（秒）',
    mfwStartupSettleTip:
      'MAS がゲームを起動した場合のみ：ウィンドウ表示後、最初のタスクを投入するまで最大この秒数待ちます。待機中は毎秒画面を確認し、内容があり 5 秒間変化がなければ早めに開始します。MaaFW の初期化は並行して進みます。Unity 製ゲームはウィンドウが出た時点ではまだ黒画面で読み込み中のことが多く、早すぎるとスクリプト側で認識異常と判定されます。ゲームが既に起動している場合は待ちません。0 で無効。',
