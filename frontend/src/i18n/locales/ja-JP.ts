...
export default {
  ...
  edit: {
    ...
    bettergiMasConfigHowTo2:
      '上の共通戦闘パーティー / 共通戦闘ストラテジーは、すべての戦闘タスク（地脈の花・秘境・ボス討伐・幽境危戦）のフォールバックです。パーティーを空欄にするとパーティーを切り替えず（タスク開始時のパーティーを維持）、ストラテジーを空欄にすると BetterGI がパーティーに応じて自動選択します。いずれかのタスクがチーム表の「戦闘シーン」に一致した場合は、その行のパーティーとストラテジーが優先されます。',
    bettergiTeamHowToTitle: '「ユーザー独立設定」でのパーティー設定',
    bettergiTeamHowTo:
