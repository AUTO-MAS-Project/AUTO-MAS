...
export default {
  ...
  edit: {
    ...
    mssFlavorQueueHint:
      '任务队列里有「活动快速战斗」时，活动期间会自动排到最前先打，非活动期间它会自动跳过。任务队列为空又没选计划表时，这一轮没有任何可执行任务：至少要勾上「悬赏试炼快速战斗」，或者选一张计划表（选了之后队列里没有它也会自动加入）。新版爬塔会排到最后，想每周只跑一次，在脚本「运行配置」的「每周完成后跳过」里选上它',
    mssFlavorPlanHint:
