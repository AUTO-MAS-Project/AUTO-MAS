...
export default {
  ...
  plan: {
    ...
    activity: {
      ...
      hint: '「倒N」= 从本期最高编号材料关往下数（倒1 即最高关）；搓玉自动识别、单列一行；指派保存在各用户配置中，此处为批量视图，改动立即生效；每个用户只会在一个槽位；「旧版序号 N」是升级前按 MAA 列表位置选的老值，移入任一槽位即改为新口径。',
      previewBanner:
