...
export default {
  ...
  plan: {
    ...
    activity: {
      ...
      hint: '一行一个用户，各自选自己这号要打的关；「倒N」= 从本期最高编号材料关往下数（倒1 即最高关），搓玉自动识别。改动立即写回该用户配置；勾选多个用户后可用下方批量条一次改开关或选关。',
      previewNotice: '当前没有进行中的活动 · 按下期已录入的关卡预解析（仅预览不注入）',
      gapNotice: '当前没有进行中的活动 · 下期活动 MAA 尚未录入，录入后此处即可指派',
      colUser: '用户',
      colSwitch: '开关',
      colStage: '选关',
      colStatus: '状态',
      filterAll: '全部',
      filterNoIntent: '未指派 {n}',
      filterAttention: '注意 {n}',
      selected: '已选 {n} 人',
      bulkKeep: '不修改',
      bulkOn: '开启',
      bulkOff: '关闭',
      bulkApply: '应用',
      bulkApplied: '已更新 {n} 个用户',
      bulkPartial: '有 {n} 个用户未更新成功：{names}',
      stateInject: '将注入',
      stateNoIntent: '未指派',
      stateSwitchOff: '已关闭',
      stateNoQuickConfig: '未开启快速配置',
      stateDisabled: '已停用',
      stateSkipped: '今日出错 · 已跳过',
      stateNoMatch: '本期无此关',
      stateNoActivity: '该服本期无活动',
      intentEmpty: '未指派（不注入）',
      intentJade: '搓玉 · {stage} · {mat}',
      intentJadeMissing: '搓玉（本期无搓玉关）',
      intentLast: '倒{n} · {stage} · {mat}',
      intentLastMissing: '倒{n}（本期无此关）',
      intentLegacy: '旧版序号 {n}（MAA 位置）',
      others: '另有 {n} 个用户跟随其他计划表或固定模式，不在此列表',
      retry: '重试',
      summary: '{n} 人跟随',
      summaryWarn: '{n} 人跟随 · {w} 项注意',
