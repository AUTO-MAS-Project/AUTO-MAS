          ...
          <a-row :gutter="24">
            <a-col :span="24">
              <a-form-item>
                <template #label>
                  <a-tooltip
                    title="按顺序加载 .py hook 文件；每个文件需定义 HOOK_META(name/description) 与 register(scope, target_cls)。加载/注册失败将被降级为警告并继续。"
                  >
                    <span class="form-label">
                      Hook 列表
                      <QuestionCircleOutlined class="help-icon" />
                ...
                <a-space style="margin-bottom: 12px" wrap>
                  <a-button size="large" class="path-button" @click="selectHookFiles">
                    <template #icon>
                      <FileOutlined />
                    </template>
                    选择 Hook 文件(.py)
                ...
                <a-list
                  bordered
                  size="small"
                  :data-source="generalConfig.Script.HookList"
                  :locale="{ emptyText: '未配置 Hook（可选）' }"
                >
                      ...
                      <div style="width: 100%">
                        ...
                        >
                          ...
                          <a-space>
                            ...
                            <a-button size="small" danger @click="removeHook(index)">移除</a-button>
                          </a-space>
                        </div>

                        <div style="margin-top: 8px">
                          <a-alert
                            v-if="getHookMeta(item)?.status === 'warning'"
                            type="warning"
                            :message="getHookMeta(item)?.warning || 'Hook 元信息读取失败'"
                            show-icon
                          />
                          <div v-else>
                            <a-tag color="blue">{{
                              getHookMeta(item)?.name || '未命名 Hook'
                            }}</a-tag>
                            <span style="margin-left: 8px; color: rgba(0, 0, 0, 0.65)">
                              {{ getHookMeta(item)?.description || '（无描述）' }}
                            </span>
                          </div>
                        </div>
                      </div>
