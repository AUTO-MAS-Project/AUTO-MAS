...
const selectEmulatorPath = async (uuid: string) => {
  try {
    ...
    // 选择任意文件
    const paths = await window.electronAPI.selectFile([{ name: t('emulator.allFiles'), extensions: ['*'] }])
...
<template>
  <div class="emulator-page">
                  ...
                  <div v-else class="devices-grid">
                    <a-table
                      ...
                      :columns="[
                        ...
                        { title: t('emulator.colName'), dataIndex: 'title', key: 'title', ellipsis: true },
                        {
    ...
    <!-- 搜索结果导入模态框 -->
    <a-modal v-model:visible="showSearchModal" :title="t('emulator.searchModalTitle')" width="600" :footer="null">
