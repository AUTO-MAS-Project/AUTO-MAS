<template>
  <div class="step-panel">
      ...
      <div class="detail-info-container">
        ...
        >
          ...
          <div class="info-items">
            ...
            >
              {{ t('init.check.executable') }}: {{ checkInfo.exeExists ? t('init.value.exists') : t('init.value.notExists') }}
            </a-tag>
            <a-tag
              v-if="checkInfo.canRun !== undefined"
              :color="checkInfo.canRun ? 'green' : 'orange'"
            >
              {{ t('init.check.runState') }}: {{ checkInfo.canRun ? t('init.value.normal') : t('init.value.abnormal') }}
            </a-tag>
            <a-tag v-if="checkInfo.version" color="blue"> {{ t('init.check.version') }}: {{ checkInfo.version }} </a-tag>
          </div>
        </div>
        ...
        >
          ...
          <div class="info-items">
            ...
            >
              {{ t('init.check.localRepo') }}: {{ checkInfo.exists ? t('init.value.exists') : t('init.value.notExists') }}
            </a-tag>
            <a-tag
              v-if="checkInfo.isGitRepo !== undefined"
              :color="checkInfo.isGitRepo ? 'green' : 'orange'"
            >
              {{ t('init.check.gitRepo') }}: {{ checkInfo.isGitRepo ? t('init.value.yes') : t('init.value.no') }}
            </a-tag>
            <a-tag
              v-if="checkInfo.isHealthy !== undefined"
              :color="checkInfo.isHealthy ? 'green' : 'orange'"
            >
              {{ t('init.check.health') }}: {{ checkInfo.isHealthy ? t('init.value.healthy') : t('init.value.abnormal') }}
            </a-tag>
        ...
        >
          ...
          <div class="info-items">
            ...
            >
              requirements.txt: {{ checkInfo.requirementsExists ? t('init.value.exists') : t('init.value.notExists') }}
            </a-tag>
            <a-tag
              v-if="checkInfo.needsInstall !== undefined"
              :color="checkInfo.needsInstall ? 'orange' : 'green'"
            >
              {{ t('init.check.needsInstall') }}: {{ checkInfo.needsInstall ? t('init.value.yes') : t('init.value.no') }}
            </a-tag>
          </div>
        </div>
        ...
        <div v-if="currentMirror || mirrorProgress" class="info-section">
          ...
          <div class="info-items">
            <a-tag v-if="currentMirror" color="blue"> {{ t('init.check.currentMirrorTag') }}: {{ currentMirror }} </a-tag>
            <a-tag v-if="mirrorProgress" color="purple">
              {{ t('init.check.attemptProgress') }}: {{ mirrorProgress.current }}/{{ mirrorProgress.total }}
            </a-tag>
          </div>
        </div>
        ...
        <div v-if="downloadSpeed || downloadSize" class="info-section">
          ...
          <div class="info-items">
            <a-tag v-if="downloadSpeed" color="green"> {{ t('init.check.downloadSpeed') }}: {{ downloadSpeed }} </a-tag>
            <a-tag v-if="downloadSize" color="cyan"> {{ t('init.check.downloaded') }}: {{ downloadSize }} </a-tag>
          </div>
        </div>
    ...
    <div v-else-if="status === 'success'" class="success-state">
      <a-result status="success" :title="t('init.step.succeeded', { title })" :sub-title="message" />
    </div>
    ...
    <div v-else-if="status === 'failed' && showMirrorSelection" class="failed-state">
      ...
      <div class="mirror-selection">
            ...
            >
              <div class="mirror-header">
                <div class="mirror-title">
                  <h4>{{ mirror.name }}</h4>
                  <a-tag v-if="mirror.recommended" color="gold" size="small">{{ t('init.env.recommended') }}</a-tag>
                </div>
              </div>
        ...
        <div class="retry-actions">
          ...
          <div v-if="countdown > 0" class="countdown-text">{{ t('init.step.autoRetryIn', { seconds: countdown }) }}</div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="status === 'failed'" class="simple-failed-state">
      <a-result status="error" :title="t('init.step.failed', { title })" :sub-title="message">
        <template #extra>
          <a-space>
            <a-button v-if="showSkipButton" @click="$emit('skip')">{{ t('init.step.skip') }}</a-button>
