...
const updateQueueItemScript = async (record: any) => {
  try {
    ...
    if (response.code === 200) {
      ...
      message.error(t('queue.toast.scriptUpdateFailed', { error: response.message || t('queue.toast.unknownError') }))
    }
  } catch (error: any) {
    const errorMsg = error instanceof Error ? error.message : String(error)
...
const addQueueItem = async () => {
  try {
    ...
    if (createResponse.code === 200 && createResponse.queueItemId) {
      ...
      message.error(t('queue.toast.addTaskFailed', { error: createResponse.message || t('queue.toast.unknownError') }))
    }
  } catch (error: any) {
    const errorMsg = error instanceof Error ? error.message : String(error)
...
const deleteQueueItem = async (itemId: string) => {
  try {
    ...
    if (response.code === 200) {
      ...
      message.error(t('queue.toast.deleteItemFailed', { error: response.message || t('queue.toast.unknownError') }))
    }
  } catch (error: any) {
    const errorMsg = error instanceof Error ? error.message : String(error)
...
const onDragEnd = async (evt: any) => {
  ...
  try {
    ...
    if (response.code === 200) {
      ...
      message.error(t('queue.toast.reorderFailed', { error: response.message || t('queue.toast.unknownError') }))
      // 如果失败，刷新数据恢复原状态
      emit('refresh')
    }
  } catch (error: any) {
