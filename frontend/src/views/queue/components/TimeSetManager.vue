        ...
        <template #item="{ element: record, index }">
          <div class="draggable-row" :class="{ 'row-dragging': loading }">
            <div class="row-cell drag-cell">
              <span class="drag-handle" :title="t('queue.item.dragSort')" :aria-label="t('queue.item.dragSort')">
...
const addTimeSet = async () => {
  try {
    ...
    if (createResponse.code === 200 && createResponse.timeSetId) {
      ...
      message.error(t('queue.toast.createTimeFailed', { error: createResponse.message || t('queue.toast.unknownError') }))
    }
  } catch (error: any) {
...
const updateTimeSetTime = async (timeSet: any) => {
  try {
    ...
    if (response.code === 200) {
      ...
      message.error(t('queue.toast.timeUpdateFailed', { error: response.message || t('queue.toast.unknownError') }))
      // 回滚时间值
...
const updateTimeSetStatus = async (timeSet: any) => {
  try {
    ...
    if (response.code === 200) {
      ...
      message.error(t('queue.toast.statusUpdateFailed', { error: response.message || t('queue.toast.unknownError') }))
      // 回滚状态
...
const updateTimeSetDays = async (timeSet: any) => {
  try {
    ...
    if (response.code === 200) {
      ...
      message.error(t('queue.toast.daysUpdateFailed', { error: response.message || t('queue.toast.unknownError') }))
    }
  } catch (error: any) {
    const errorMsg = error instanceof Error ? error.message : String(error)
...
const deleteTimeSet = async (timeSetId: string) => {
  try {
    ...
    if (response.code === 200) {
      ...
      message.error(t('queue.toast.deleteTimeFailed', { error: response.message || t('queue.toast.unknownError') }))
    }
  } catch (error: any) {
    const errorMsg = error instanceof Error ? error.message : String(error)
...
const onDragEnd = async (evt: any) => {
  ...
  try {
    ...
    if (response.code === 200) {
      ...
      message.error(t('queue.toast.reorderTimeFailed', { error: response.message || t('queue.toast.unknownError') }))
      // 如果失败，刷新数据恢复原状态
      emit('refresh')
    }
  } catch (error: any) {
