...
def test_enabled_one_dragon_task_names_follow_queue_order_and_skip_disabled():
    # 通知分步表把 BGI 的「一条龙任务执行: X/N」序号还原成任务名时的顺序表：内置任务按
    # 队列顺序、只含启用项；自定义配置组由 BGI 另开编号（配置组任务执行: X/M），不在表内
    result = one_dragon.apply_groups(
        config={},
        enabled=["领取邮件", "自动幽境危战", "自动地脉花"],
        queue=_queue_with(
            [
                ("领取邮件", True),
                ("提瓦特记事本", True),
    ...
    assert one_dragon.enabled_one_dragon_task_names(result) == ["领取邮件", "自动幽境危战"]


def test_enabled_one_dragon_task_names_keep_duplicate_instances():
    # 同名多实例（复制相同）各占一项：BGI 也逐个编号，不能按名去重，否则名字整体错位
    result = one_dragon.apply_groups(
        config={},
        enabled=["自动秘境"],
        queue=_queue_with([("自动秘境", True), ("自动秘境", True)]),
    )

    assert one_dragon.enabled_one_dragon_task_names(result) == ["自动秘境", "自动秘境"]
