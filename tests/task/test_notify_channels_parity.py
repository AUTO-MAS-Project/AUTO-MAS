...
SCENARIOS: dict[str, tuple[dict, dict]] = {
    ...
    "result_never": (_g(SendTaskResultTime="不推送"), _u()),
...
def _patched(mod, fake_cfg, notify):
    """把 Config/Notify 替换掉——脚本模块和通知中间层都要打。

    去重后 fan-out 发生在 app.services.notify_dispatch 里, 那里有自己的 Config/Notify
    引用; 只打脚本模块的话假对象会被绕过, 测试会静默失效(甚至真去连 SMTP)。
    重构后脚本模块只留 Config(取模板), 不再引用 Notify, 故按实际存在的属性打桩。
    """
    with contextlib.ExitStack() as stack:
        for target in (mod, notify_dispatch):
